...
const PokemonAbilities: FC<{ data: Pokemon }> = ({ data }) => {
  const abilitiesData = Object.entries(data?.abilities).filter((item) => item !== null && item[0] !== "__typename");
  ...
  if (abilitiesData) {
    abilitiesContent = abilitiesData.map((ability: any, index: number) => {
      return (
        <div
        ...
        >
          <p className={classes.abilityItem__name}>{ability[1]?.name}</p>
        </div>
      );
    });
  }
  ...
  return (
    <div data-testid="pokemon-content-abilities" className="pokemon-page-card">
