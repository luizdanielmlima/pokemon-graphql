...
const PokemonMiscInfo: FC<{ data: Pokemon }> = ({ data }) => {
  ...
  return (
    <div data-testid="pokemon-content-info" className="pokemon-page-card" style={{ alignSelf: "self-start" }}>
      <p className="pokemon-page-title">MISC INFO</p>
      <div className={classes.items}>
        <InfoItem label={`Base EXP:`} value={data?.baseStatsTotal} unit={null} />
        <InfoItem label={`Height:`} value={convUnit(data?.height)} unit={"m"} />
        <InfoItem label={`Weight:`} value={convUnit(data?.weight)} unit={"kg"} />
        <InfoItem label={`Species:`} value={data?.species} unit={null} />
        <InfoItem label={`Evolution Level:`} value={data?.evolutionLevel} unit={null} />
      </div>
    </div>
  );
};
